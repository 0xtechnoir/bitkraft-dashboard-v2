...
data = list(btc_collection.find({"timestamp": {"$gte": min_date_timestamp}}, {'_id': 0}))
...
def display_btc_futures_agg_open_interest_chart():
    return html.Div(
        children=dcc.Graph(
            id="btc_agg_open_interest_fig", 
            figure=update_chart(),
...
@app.callback(
    Output('btc_agg_open_interest_fig', 'figure'),
    Input("btc_agg_open_interest_fig", "relayoutData"),
)
def update_chart(relayoutData=None):
    ...
    fig.update_layout(
        title=dict(
            text="BTC Futures Aggregated Open Interest",
            x=0.08,
            xanchor="left",
        ),
        colorway=["#17B897"],
